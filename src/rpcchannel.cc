 ...
 *  所有通过stub代理对象调用的rpc方法，都通过RpcChannel::CallMethod方法
 *  统一做rpc方法调用的数据序列化和网络发送
 *  @ const google::protobuf::MethodDescriptor* method 是 protobuf自动帮我们填好的对于method的descriptor
 *   
 */
                            ...
                            google::protobuf::Closure* done)
{
    ...
    send_rpc_str.insert(0, std::string((char*)&header_size, 4)); // header_size reinterpret_cast<char*>
    ...
    if(!response->ParseFromArray(recv_buf, recv_size))
    {
        ...
        controller->SetFailed("response_str parse error!");
        return;
    }
}